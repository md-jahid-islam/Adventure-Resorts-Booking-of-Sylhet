...
import { 
 ...
 export function AdPopup() {
  ...
  const handleWhatsAppClick = () => {
    ...
    window.open(`https://wa.me/message/O3CTLULQDASLC1=${message}`, '_blank');
  };
      ...
      <DialogContent className={`sm:max-w-md p-0 overflow-hidden ${isMobile ? 'w-[90%] max-h-[80vh]' : ''}`}>
        <motion.div
          ...
          <div className="relative">
            <img src="/lovable-uploads/f97f4d91-56e4-4e2f-bb73-93760030da48.png" alt="Day Package"className={`w-full object-cover ${isMobile ? 'h-36' : 'h-48'}`}/>
            <button onClick={() => setIsOpen(false)}className="absolute top-2 right-2 rounded-full bg-black/50 p-1 text-white hover:bg-black/70 transition-colors"><X className="h-4 w-4" />
            </button>
          </div>
