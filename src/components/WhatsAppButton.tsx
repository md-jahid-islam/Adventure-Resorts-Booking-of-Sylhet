...
import { cn } from '@/lib/utils';
 ...
 // ============ Updated to use the specified number ============ //
  const WhatsAppButton: React.FC = () => {
  const phoneNumber = '+8801540587085'; 
  const message = encodeURIComponent("Hi! I'm interested in booking an adventure at Dandeli.");
  const whatsappUrl = `https://wa.me/message/O3CTLULQDASLC1${phoneNumber}?text=${message}`;

  return (
    <a href={whatsappUrl} target="_blank" rel="noopener noreferrer"className={cn( "fixed bottom-6 left-6 z-50","w-14 h-14 bg-[#25D366] hover:bg-[#20BD5C] rounded-full shadow-lg","flex items-center justify-center","transition-all duration-300 hover:scale-110 animate-bounce")}
